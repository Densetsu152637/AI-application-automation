...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const db = appDb(); try { const result = controlRun(db, (await context.params).id, 'resume_run', expected); return NextResponse.json(result, { status: result.status ?? 202 }); } finally { db.close(); }
}
