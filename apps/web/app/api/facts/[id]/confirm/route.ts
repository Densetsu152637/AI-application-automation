...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const body = await request.json().catch(() => null) as { value?: unknown; reason?: unknown } | null;
  if (!body || !('value' in body) || typeof body.reason !== 'string' || body.reason.length > 2000) return apiError('VALIDATION_FAILED', 400);
  const { id } = await context.params; const db = appDb(); const hash = bodyHash(body);
  try {
    const scope = idempotencyScope(request, `/api/v1/facts/${id}/confirm`, 'administrator'); const prior = findIdempotency(db, scope, key, hash); if (prior === 'CONFLICT') return apiError('IDEMPOTENCY_CONFLICT', 409, undefined, { entityId: id }); if (prior) return apiOk(prior.body, prior.status);
    ...
    const data = { id, revision, status: 'confirmed' }; db.transaction(() => { db.prepare("UPDATE applicant_facts SET revision=?,value_json=?,status='confirmed',confirmed_at=?,confirmed_by=?,updated_at=? WHERE id=?").run(revision, JSON.stringify(body.value), now, 'administrator', now, id); saveIdempotency(db, scope, key, hash, 200, data, now); })();
    return apiOk(data);
  } finally { db.close(); }
}
